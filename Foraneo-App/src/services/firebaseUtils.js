	...
	try {
		if (auth.currentUser) {
			...
			await updateDoc(userRef, {
				...
				totalAvailable: increment(transactionData.type === 'Income' ? transactionData.value : -transactionData.value),
			});
...
export const calculateRecipeSavings = (completedRecipes) => {
	const deliveryCost = 21000;

	const totalIngredientCost = completedRecipes.reduce((total, recipe) => {
		const ingredientCost = Object.values(recipe.ingredientPrices).reduce((subtotal, price) => {
			return subtotal + (parseFloat(price) || 0);
		}, 0);
		return total + ingredientCost;
	}, 0);

	const numberOfRecipes = completedRecipes.length;

	const totalDeliveryCost = numberOfRecipes * deliveryCost;

	const savings = totalDeliveryCost - totalIngredientCost;

	return { totalIngredientCost, savings };
};
